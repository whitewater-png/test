...
- 座り手首・座り腕の確定値(2026-07-16、実描画+ズーム画像で確認済み):
  `armX=0.85, loArmY=0.55, handX=2.2`(他は既存値のまま: `armY=0.15, armZ=1.5, loArm=1.35,
  handY=0.50, handZ=1.31`)。旧値(`armX=0.5, loArmY=0.0, handX=1.54`)は手が太ももから完全に浮いて
  横に投げ出される状態だった(ユーザーからの再指摘で発覚。前回の「手のひら下向き確認済み」は
  手首の**向き**だけを見ており、手の**位置**(太ももに届いているか)を見落としていた教訓)
...
| `handX` | 2.2 | 手首の曲げ(指先を太もも面まで折り込む主レバー) |
| `handY` | 0.50 | 手首のひねり |
| `handZ` | 1.31 | 手首の左右振り |
