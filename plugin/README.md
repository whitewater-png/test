...
### 主な用途: トランスフォームでの拡大（パンチイン）の画質改善

このプラグインが最も効果を発揮する典型的なシナリオは、**4Kシーケンス上の
4K素材を「モーション/トランスフォーム」エフェクトのスケールでパンチイン
（部分拡大）したときに映像が荒くなる問題**を、AIで事前にディテールを補って
から拡大することで軽減することです。Premiereはエフェクトスタック内で
本プラグインのようなエフェクト → モーション/トランスフォームの順に処理する
ため、本プラグインが出力バッファをScale倍に拡大できれば（`I_EXPAND_BUFFER`
がホストに尊重される場合）、後段のトランスフォームによる拡大は元の低解像度
フレームではなく、このプラグインが生成した高解像度バッファからサンプリング
されるようになり、単純な拡大よりもシャープな結果になります。

**手順:**

1. 4Kクリップ（4Kシーケンス上）に「AI Upscale」エフェクトを適用します。
2. `Scale` パラメータを、後段でモーション/トランスフォームにかける予定の
   拡大率に合わせて選びます。目安として:
   - 拡大率が概ね200%程度までのパンチイン → `Scale: 2x`
   - 拡大率が概ね400%程度までのパンチイン → `Scale: 4x`
   （Scaleは2x/4xの2択のみです。中間の拡大率でも、実際のパンチイン率に近い
   方を選んでください。）
3. 同じクリップに「モーション」エフェクト（またはトランスフォームエフェクト）
   を追加し、`スケール` を目的のパンチイン率に設定して拡大します。
4. エフェクトの順序は「AI Upscale」が「モーション/トランスフォーム」より
   **前**（上）にあることを確認してください。順序が逆だと、AI Upscaleが
   トランスフォーム後の（既に拡大されて荒れた）フレームを処理することに
   なり、狙った効果が得られません。

**ホストが拡大を尊重しない環境での動作:** Premiere/After Effectsのバージョン
や設定によっては、エフェクトが要求したバッファ拡張（`I_EXPAND_BUFFER`）が
尊重されず、出力ワールドが入力と同じサイズのまま渡されることがあります
（詳細は本ファイル冒頭のコード内コメントおよび「安定運用ガイド」手順6を
参照）。この場合でも本プラグインは失敗せず、出力サイズを入力サイズのまま
に保った「ディテール再生成」モードとして動作します（AIで細部を補ったフレーム
を同解像度で返す）。この場合、後段のトランスフォーム拡大は単純な補間よりは
改善されますが、真に高解像度なバッファからのサンプリングにはならないため、
「主な用途」で説明した理想的な効果（拡大自体のシャープさ向上）は限定的に
なります。現在どちらの状態で動作しているかは、ログの `expand_status`
（`expanded` / `not-expanded` / `custom`）で確認できます（「安定運用ガイド」
手順6参照）。
...
5. **異常サイズ入力の確認**: ログに `"exceeds maximum supported dimension"`
   ...
   定数を見直してください。
6. **バッファ拡張（I_EXPAND_BUFFER）がホストに尊重されたかの確認**: ログの
   `"HandleRender: size combo changed"` 行末尾の `expand_status=` を見てください。
   `expanded`（出力ワールド＝入力ワールド×Scale。理想動作）、`not-expanded`
   （出力ワールド＝入力ワールドと同サイズ。ディテール再生成のみのフォール
   バック動作）、`custom`（それ以外の組み合わせ。ホスト側の特殊な事情による
   もので、想定外のサイズ関係が疑われる場合はこの行と前後の入出力サイズを
   添えて報告してください）のいずれかが1語で出力されます。詳しくは次節
   「使い方ガイド」も参照してください。
